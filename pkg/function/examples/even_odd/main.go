...
func handle(_ context.Context, keys []string, d functionsdk.Datum) functionsdk.Messages {
	...
	if num, err := strconv.Atoi(string(msg)); err != nil {
		...
		return functionsdk.MessagesBuilder().Append(functionsdk.NewMessage(msg).WithKeys([]string{"even"}))
	} else {
		return functionsdk.MessagesBuilder().Append(functionsdk.NewMessage(msg).WithKeys([]string{"odd"}))
	}
}
