...
func FilterEventTime(keys []string, d functionsdk.Datum) functionsdk.MessageTs {
	...
	if d.EventTime().Before(janFirst2022) {
		...
		return functionsdk.MessageTsBuilder().Append(functionsdk.NewMessageT(janFirst2022, d.Value()).WithKeys([]string{"within_year_2022"}))
	} else {
		return functionsdk.MessageTsBuilder().Append(functionsdk.NewMessageT(janFirst2023, d.Value()).WithKeys([]string{"after_year_2022"}))
	}
}
