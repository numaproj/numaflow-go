...
func FilterEventTime(key string, d functionsdk.Datum) functionsdk.MessageTs {
	...
	if d.EventTime().Before(janFirst2022) {
		...
		return functionsdk.MessageTsBuilder().Append(functionsdk.MessageTTo(janFirst2022, "within_year_2022", d.Value()))
	} else {
		return functionsdk.MessageTsBuilder().Append(functionsdk.MessageTTo(janFirst2023, "after_year_2022", d.Value()))
	}
}
