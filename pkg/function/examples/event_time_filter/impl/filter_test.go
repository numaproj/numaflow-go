...
func Test_FilterEventTime(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			expectedOutput: functionsdk.MessageTs{
				functionsdk.NewMessageT([]byte("test-data")).WithKeys([]string{"within_year_2022"}).WithEventTime(janFisrt2022),
			},
		},
		{
			...
			expectedOutput: functionsdk.MessageTs{
				functionsdk.NewMessageT([]byte("test-data")).WithKeys([]string{"after_year_2022"}).WithEventTime(janFirst2023),
			},
		},
	}
