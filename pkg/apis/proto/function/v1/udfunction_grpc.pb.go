...
type UserDefinedFunctionClient interface {
	...
	MapStreamFn(ctx context.Context, in *DatumRequest, opts ...grpc.CallOption) (UserDefinedFunction_MapStreamFnClient, error)
...
func (c *userDefinedFunctionClient) MapStreamFn(ctx context.Context, in *DatumRequest, opts ...grpc.CallOption) (UserDefinedFunction_MapStreamFnClient, error) {
	stream, err := c.cc.NewStream(ctx, &UserDefinedFunction_ServiceDesc.Streams[0], "/function.v1.UserDefinedFunction/MapStreamFn", opts...)
	if err != nil {
		return nil, err
	}
	x := &userDefinedFunctionMapStreamFnClient{stream}
...
type userDefinedFunctionMapStreamFnClient struct {
	grpc.ClientStream
}
...
func (c *userDefinedFunctionClient) ReduceFn(ctx context.Context, opts ...grpc.CallOption) (UserDefinedFunction_ReduceFnClient, error) {
	stream, err := c.cc.NewStream(ctx, &UserDefinedFunction_ServiceDesc.Streams[1], "/function.v1.UserDefinedFunction/ReduceFn", opts...)
	if err != nil {
		return nil, err
	}
...
type UserDefinedFunctionServer interface {
	...
	// MapStreamFn applies a function to each datum element and returns a stream.
	MapStreamFn(*DatumRequest, UserDefinedFunction_MapStreamFnServer) error
	// MapTFn applies a function to each datum request element.
	// In addition to map function, MapTFn also supports assigning a new event time to datum.
	// MapTFn can be used only at source vertex by source data transformer.
...
func (UnimplementedUserDefinedFunctionServer) MapStreamFn(*DatumRequest, UserDefinedFunction_MapStreamFnServer) error {
...
func _UserDefinedFunction_MapStreamFn_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(DatumRequest)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(UserDefinedFunctionServer).MapStreamFn(m, &userDefinedFunctionMapStreamFnServer{stream})
}
...
type userDefinedFunctionMapStreamFnServer struct {
	grpc.ServerStream
}
	...
	Streams: []grpc.StreamDesc{
		{
			...
			Handler:       _UserDefinedFunction_MapStreamFn_Handler,
			ServerStreams: true,
		},
		{
			StreamName:    "ReduceFn",
