...
[numaflow-go#228](https://github.com/numaproj/numaflow-go/issues/228)).

To get "blackhole" semantics without leaking memory, this example emits a *drop* message for every
datum using `accumulator.MessageToDrop`. A drop message is not forwarded downstream, but it still
lets the framework advance the watermark and release the tracked state for that datum.
...
```shell
  kubectl apply -f manifest/blackhole-pipeline.yaml
```
...
```shell
  kubectl logs -f blackhole-log-sink-0-xxxx
```
